...
registerMooseObject("StochasticToolsApp", ChangeSubdomainTransfer);
...
{
  TIME_SECTION(
      "ChangeSubdomainTransfer::execute()", 5, "Performing transfer with a user object");
  ...
  switch (_current_direction)
  {
    case TO_MULTIAPP:
      for (unsigned int i_to = 0; i_to < _to_problems.size(); ++i_to)
      {
        ...
        const ChangeSubdomainAssignment * csa_uo = dynamic_cast<const ChangeSubdomainAssignment *>(uo);
        if (!csa_uo)
