 ...
 */
void
sa_type_base::optimize(sa_type_base & my_sa)
{
  int i, step;
  ...
  else if (sa_cont_type * sa_ptr = dynamic_cast<sa_cont_type *>(&my_sa))
  {
    ...
    if (sa_ptr->damping <= 1.0E-15)
      sa_ptr->damping = abs(sa_ptr->smax - sa_ptr->smin) / 2.0E+0;
  }
  else
  {
  ...
  while (step < my_sa.max_step && t_curr > my_sa.t_min)
  {
    ...
    {
      sa_ptr->get_neigh(sa_ptr->state_curr,
                        sa_ptr->state_neigh,
                        sa_ptr->damping,
                        sa_ptr->smax,
                        sa_ptr->smin,
    ...
    if (!my_sa.mon_cool)
      t_curr = t_curr * (1.0 + (e_curr - my_sa.e_best) / e_curr);
    ...
    if (abs(t_curr) <= my_sa.resvar)
    {
      ...
      else if (sa_cont_type * sa_ptr = dynamic_cast<sa_cont_type *>(&my_sa))
      {
        for (i = 0; i < my_sa.state_size; i++)
        {
          ...
          if (sa_ptr->damp_dyn)
            sa_ptr->damping = sa_ptr->damping / 2.0;
        }
      }
    }
  }
 ...
 */
void
set_cooling(sa_type_base & my_sa)
{
 ...
 */
double
lin_mult_cool(sa_type_base & my_sa, int k)
{
 ...
 */
double
exp_mult_cool(sa_type_base & my_sa, int k)
{
 ...
 */
double
log_mult_cool(sa_type_base & my_sa, int k)
{
 ...
 */
double
quad_mult_cool(sa_type_base & my_sa, int k)
{
 ...
 */
double
lin_add_cool(sa_type_base & my_sa, int k)
{
  return my_sa.t_min +
         (my_sa.t_max - my_sa.t_min) * (my_sa.max_step * 1.0E0 - k) / (my_sa.max_step * 1.0E0);
}
 ...
 */
double
quad_add_cool(sa_type_base & my_sa, int k)
{
  return my_sa.t_min + (my_sa.t_max - my_sa.t_min) *
                           pow((my_sa.max_step * 1.0E0 - k) / (my_sa.max_step * 1.0E0), 2);
}
 ...
 */
double
exp_add_cool(sa_type_base & my_sa, int k)
{
  return my_sa.t_min + (my_sa.t_max - my_sa.t_min) /
                           (1.0E0 + exp(2.0E0 * log(my_sa.t_max - my_sa.t_min) *
                                        (k - 0.5E0 * my_sa.max_step) / (my_sa.max_step * 1.0E0)));
}
 ...
 */
double
trig_add_cool(sa_type_base & my_sa, int k)
{
  return my_sa.t_min +
         0.5E0 * (my_sa.t_max - my_sa.t_min) * (1.0E0 + cos(k * M_PI / (my_sa.max_step * 1.0E0)));
}
 ...
 */
void
sa_comb_type::get_neigh(int * s_curr, int * s_neigh, int size_state)
{
  int i, j1, j2;
  double diff;
 ...
 */
void
sa_cont_type::get_neigh(double * s_curr,
                        double * s_neigh,
                        double damping,
                        double smax,
                        double smin,
 ...
 */
double
accept_prob(double e_current, double e_neigh, double t_current)
{
 ...
 */
void
sa_type_base::clear(sa_type_base & my_sa)
{
