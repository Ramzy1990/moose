...
TEST(SimulatedAnnealingTests, TravelingSalesman)
{
  ...
  for (i = 0; i < num_customers; i++)
  {
    ...
    for (j = 0; j < prob_dim; j++)
    {
      ...
      if (cust_min >= cust_locs[i][j])
        cust_min = cust_locs[i][j];
      if (cust_max <= cust_locs[i][j])
        cust_max = cust_locs[i][j];
    }
  }

  // for a 1D problem, this is the optimal path length
  if (prob_dim == 1)
    sort_best = cust_max - cust_min;
